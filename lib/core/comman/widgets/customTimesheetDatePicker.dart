...
class _PeriodPickerDialogState extends State<PeriodPickerDialog> {
  ...
  void initState() {
    ...
    _selectedRange = widget.getRangeFromConfig(
      widget.periodType,
      widget.initialDate,
      weekStart: !widget.status ? widget.weekStart : '',
      monthStart: !widget.status ? widget.monthStart : '',
    );
    print(
      'Initial selected range: ${_fmt(_selectedRange!.start)} - ${_fmt(_selectedRange!.end)}',
    );
    print(
      'Initial date: ${_fmt(widget.initialDate)} focused month: ${_monthLabel(_focusedMonth)}  period type: ${widget.periodType} weekStart: ${widget.weekStart} monthStart: ${widget.monthStart} ',
    );
  }
  ...
  void _onDayTapped(DateTime date) {
    ...
    final clampedEnd = range.end.isAfter(widget.lastDate)
        ? widget.lastDate
        : range.end;
    final clampedStart = range.start.isAfter(clampedEnd)
        ? clampedEnd
        : range.start;
  ...
  bool _isStart(DateTime date) =>
      _selectedRange != null && _isSameDay(date, _selectedRange!.start);
  bool _isEnd(DateTime date) =>
      _selectedRange != null && _isSameDay(date, _selectedRange!.end);
  ...
  Widget _buildWeekdayHeaders() {
    ...
    final ordered = [
      ...allDays.sublist(startIdx),
      ...allDays.sublist(0, startIdx),
    ];
  ...
  Widget _buildCalendarGrid(ColorScheme colorScheme) {
    ...
    final cells = <DateTime?>[
      ...
      ...List.generate(
        daysInMonth,
        (i) => DateTime(firstOfMonth.year, firstOfMonth.month, i + 1),
      ),
  ...
  Widget _buildDayCell(DateTime date, ColorScheme colorScheme) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: cellBg, borderRadius: radius),
