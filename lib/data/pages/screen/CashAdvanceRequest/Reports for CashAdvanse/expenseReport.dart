...
class CashAdvanceMISReports extends StatefulWidget {
  const CashAdvanceMISReports({Key? key}) : super(key: key);

  @override
  _CashAdvanceMISReportsState createState() =>
      _CashAdvanceMISReportsState();
}

class _CashAdvanceMISReportsState extends State<CashAdvanceMISReports> {
  int _currentStep = 0;
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> screens = [
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
  title: Text( '${AppLocalizations.of(context)!.cashAdvance} ${AppLocalizations.of(context)!.reports}'),          centerTitle: true,
        ),
        body: Padding(  
        ...
        bottomNavigationBar: Container(
          // Optional: some padding if you want
...
class _BasicFiltrationScreenState extends State<BasicFiltrationScreen> {
  late final reportModel = Provider.of<ReportModel>(context);
  @override
  void initState() {
    super.initState();

    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<ReportModel>(
        context,
        listen: false,
      ).updateFunctionalAreaExpenseReport("Cash Advance Requisition");
    });
  }
          ...
          children: [
            ...
            TextFormField(
              ...
              validator: (value) {
                ...
                final reportModel = Provider.of<ReportModel>(
                  context,
                  listen: false,
                );
                ...
                if (reportModel.fromDateCtrl.text.isNotEmpty) {
                  ...
                  if (toDate.isBefore(fromDate)) {
                    return AppLocalizations.of(context)!.toDateValidation;
                  }
                }
            ...
            //   items: const ['Expense Requisition'],
            //   displayText: (e) => e ?? '',
            ...
            CustomDropdownList<String>(
              ...
              items: reportModel.allDatasets.map((e) => e.toString()).toList(),

              // ✅ Display text
              displayText: (e) => e ?? '',

              // ✅ IMPORTANT: handle null safely
              selectedValue: reportModel.sortByExpense.toString(),

              onChanged: (value) {
                if (value != null) {
                  reportModel.sortByExpense = value;
            ...
            CustomDropdownList<String>(
              ...
              displayText: (e) => e ?? '',
              selectedValue: reportModel.sortOrder == 'desc'
                  ? 'Ascending'
              ...
              onChanged: (value) {
                if (value != null) {
                  final sortVal = value == 'Ascending' ? '' : 'desc';
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildCheckTile(
              value: reportModel.transData,
              title: AppLocalizations.of(context)!.transData,
              onChanged: (v) => setState(() => reportModel.transData = v),
            ),
            _buildCheckTile(
              value: reportModel.documentAttachments,
              title: AppLocalizations.of(context)!.documentAttachments,
              onChanged: (v) =>
                  setState(() => reportModel.documentAttachments = v),
            ),
            _buildCheckTile(
              value: reportModel.accountingDistributions,
              title: AppLocalizations.of(context)!.accountDistribution,
              onChanged: (v) =>
                  setState(() => reportModel.accountingDistributions = v),
            ),
            _buildCheckTile(
              value: reportModel.expenseCategoryCustomFieldValues,
              title: AppLocalizations.of(context)!.expenseCategoryCustomFields,
              onChanged: (v) => setState(
                () => reportModel.expenseCategoryCustomFieldValues = v,
              ),
            ),
            _buildCheckTile(
              value: reportModel.transCustomfieldValues,
              title: AppLocalizations.of(context)!.transCustomFieldsValues,
              onChanged: (v) =>
                  setState(() => reportModel.transCustomfieldValues = v),
            ),
            _buildCheckTile(
              value: reportModel.headerCustomfieldValues,
              title: AppLocalizations.of(context)!.headerCustomFieldsValues,
              onChanged: (v) =>
                  setState(() => reportModel.headerCustomfieldValues = v),
            ),
            _buildCheckTile(
              value: reportModel.activityLog,
              title: AppLocalizations.of(context)!.activityLog,
              onChanged: (v) => setState(() => reportModel.activityLog = v),
            ),
            _buildCheckTile(
              value: reportModel.workFlowHistory,
              title: AppLocalizations.of(context)!.workflowHistory,
              onChanged: (v) => setState(() => reportModel.workFlowHistory = v),
            ),
              ...
              child: ElevatedButton(
                ...
                onPressed: isLoading
                    ...
                    : () async {
                        ...
                        if (reportModel.transData) selected.add('ExpenseTrans');
                        if (reportModel.documentAttachments) {
                          selected.add('DocumentAttachments');
                        }
                        if (reportModel.accountingDistributions) {
                          selected.add('AccountingDistributions');
                        }
                        if (reportModel.expenseCategoryCustomFieldValues) {
                          selected.add('ExpenseCategoryCustomFieldValues');
                        }
                        if (reportModel.transCustomfieldValues) {
                          selected.add('TransCustomfieldValues');
                        }
                        if (reportModel.headerCustomfieldValues) {
                          selected.add('HeaderCustomfieldValues');
                        }
                        if (reportModel.workFlowHistory) {
                          selected.add('WorkFlowHistory');
                        }
                        if (reportModel.activityLog) {
                        ...
                        if (mounted) {
                          ...
                          await reportModel.callMISReportAPI(context);
                        }
