...
class _TimeSheetRequestPageState extends State<TimeSheetRequestPage> {
  ...
  bool _validateForm() {
    ...
    final bool attachmentsRequired =
        controller.ruleConfig?.attachmentsRequired ?? false;
    if (attachmentsRequired && controller.uploadedImages.isEmpty) {
      controller.showAttachmentError.value = true;
      isValid = false;
    } else {
      controller.showAttachmentError.value = false;
    }
    // Validate line items
...
final config = await controller.getRuleConfig(
  ...
  toDate: toDateOrg,
    ...
    /// Priority:
    /// 1. ruleConfig.frequency
    /// 2. config.entryFrequency
    /// 3. Default => Day

    if (controller.ruleConfig?.frequency != null &&
        controller.ruleConfig!.frequency!.isNotEmpty &&
        !widget.status) {
      controller.periodType.value = controller.getPeriodTypeForUI(
        controller.ruleConfig!.frequency!,
      );

      print(
        "Setting periodType from ruleConfig.frequency: "
        "${controller.periodType.value}${periodTypes} ${config == null}${widget.status}",
      );
    } else if (config == null || widget.status) {
      print(
        "Config is null or widget.status is true, setting default periodType to 'Day'",
      );
      // controller.periodType.value = controller.getPeriodTypeForUI(
      ...
      print(
        "Setting periodType from entryFrequency: "
        "${controller.periodType.value}",
      );
    } else {
      print(
        "Skip updating periodType because "
        "config != null && !widget.status",
      );
    }
          ...
          child: Column(
            ...
            children: [
              ...
              Obx(() {
                ...
                if (controller.timeSheetID.text.isEmpty && hideField) {
        ...
        Obx(() {
          return Column(
            ...
            children: [
              ...
              Obx(
                () => controller.showAttachmentError.value
                    ? const Text(
...
case 'date&time':
  ...
  Widget _buildInlineValueDisplay(
    ...
    switch (fieldType) {
      ...
      case 'date&time':
        return Row(
          children: [
            Icon(Icons.calendar_today, size: 10, color: Colors.blue.shade400),
      ...
      case 'email':
        return Row(
          children: [
            Icon(Icons.email_outlined, size: 10, color: Colors.blue.shade400),
            const SizedBox(width: 3),
            Expanded(
              child: Text(
                displayText,
      ...
      case 'url':
        return Row(
          children: [
            Icon(Icons.link, size: 10, color: Colors.blue.shade400),
            const SizedBox(width: 3),
            Expanded(
              child: Text(
                displayText,
  ...
  Widget _buildApprovalActionButtons() {
    ...
    return Column(
      children: [
        if (stepType == "Review" && controller.sheetEnable.value)
          Row(
            children: [
              Obx(() {
        ...
        if (stepType == "Approval" && controller.sheetEnable.value)
          Row(
            children: [
              Obx(() {
  ...
  Future<void> _pickDateRange() async {
    ...
    if (result != null) {
      ...
      controller.fetchTasksTimeSheet(
        fromDate: toStartOfDayUtc(result.start),
        toDate: toEndOfDayUtc(result.end),
      );
      controller.loadTimeSheetRange(
        fromDate: toStartOfDayUtc(result.start),
        toDate: toEndOfDayUtc(result.end),
  ...
  DateTimeRange _getDateRangeByPeriod(
    ...
    int extractDay(String value) {
      ...
      if (trimmed == "firstday") return 1;
      if (trimmed == "lastday") return 31;
...
class _HourItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final entryKey = toMillisecondsWithTimezone(
      DateTime(item.entryDate.year, item.entryDate.month, item.entryDate.day),
    );
    ...
    final bool hasEntry = entry != null; // true = entry EXISTS
    final bool isNewEntry = entry == null; // true = already exists
    final isRunning = line.timerRunning.value;
    final seconds = line.elapsedSeconds.value;
    print("⏱ isRunning: $isRunning, seconds: $seconds");
    ...
    final double limit = controller.ruleConfig!.limitForPastDate;
    ...
    print("🚫 isCreateMode: $isCreateMode, hasLock: ${difference > 0}");
    ...
    final bool isAllowedPastDay = isNewEntry
        ? (withinPastLimit &&
              !isLockedByOldRecord) // No entry: allow if within limit & not locked
        : withinPastLimit;
    final bool anyTimerRunning = controller.lineItems.any(
      (line) => line.timerRunning.value,
    );

    /// ✅ Check if THIS line's timer is running
    final bool thisLineRunning =
        controller.lineItems[lineIndex].timerRunning.value;

    /// ✅ Check which line is currently running
    final int runningLineIndex = controller.lineItems.indexWhere(
      (line) => line.timerRunning.value,
    );
    final bool weekendsAllowed =
        controller.ruleConfig?.isWeekendsAllowed ?? false;
    final bool holidaysAllowed =
        controller.ruleConfig?.isHolidaysAllowed ?? false;

    print(
      "controller.lineItems[lineIndex].timerRunning.value: ${controller.lineItems[lineIndex].timerRunning.value}, holidaysAllowed: $holidaysAllowed, item.weekend: ${item.weekend}, item.holiday: ${item.holiday}",
    );

    /// 🚫 FINAL DISABLE CONDITION
    final bool isDisabled =
        (item.weekend && !weekendsAllowed) ||
        (item.holiday && !holidaysAllowed) ||
        isFuture ||
        (!isToday && !isAllowedPastDay) ||
        (!isToday && isRunning) ||
        (!isToday && thisLineRunning);
    ...
    print(
      "controller.ruleConfig!.limitForPastDate: ${controller.ruleConfig!.limitForPastDate}",
    );
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                if (showTimerButton &&
                    controller.lineItems[lineIndex].timerRunning.value &&
                    !controller.showCancelIcon.value &&
                    !isDisabled)
                  Obx(() {
                    final isRunning =
                        controller.lineItems[lineIndex].timerRunning.value;
                    final seconds =
                        controller.lineItems[lineIndex].elapsedSeconds.value;
            ...
            Obx(() {
              ...
              if (entry?.totalHours == null || isRunning)
                return const SizedBox.shrink();
                ...
                children: [
                  TextButton(
                    onPressed: isDisabled
                        ...
                        : () {
                            final double hours =
                                double.tryParse(entry?.totalHours ?? '0') ??
                                0.0;

                            final freshEntry =
                                controller.timeEntries[lineIndex]?[entryKey];
                            final entryDistributions =
                                freshEntry?.accountingDistributions ?? [];

                            controller.split.clear();

                            if (entryDistributions.isNotEmpty) {
                              controller.split.assignAll(
                                entryDistributions.map((e) {
                              ...
                              builder: (context) => Padding(
                                ...
                                child: SingleChildScrollView(
                                  child: AccountingDistributionWidgetHours(
                                    ...
                                    lineAmount: hours,
                                    isEnable: controller.sheetEnable.value,
                                    ...
                                    onDistributionChanged: (newList) {
                                      ...
                                      if (currentEntry != null) {
                                        ...
                                        controller.timeEntries[lineIndex] ??=
                                            {};
                                        controller
                                                .timeEntries[lineIndex]![entryKey] =
                                            TimeEntryModel(
                    ...
                    child: const Icon(Icons.call_split, size: 12),
                  ),
                ],
              );
            }),
...
// ================= BIWEEKLY =================
  ...
  if (weekStart.trim().isEmpty) {
    ...
    // ================= MONTHLY =================
      ...
      if (monthStart.trim().isEmpty) {
        /// ✅ No config: tapped date IS the start, end = last day of same month
        startDate = date;
        endDate = DateTime(
          date.year,
          date.month + 1,
...
class LineCustomFieldSheet extends StatefulWidget {
  ...
  final VoidCallback? onSaved; // ✅ NEW
...
class _LineCustomFieldSheetState extends State<LineCustomFieldSheet> {
  ...
  CustomDropdownValue? _selectedDropdownValue;
  ...
  String get _fieldType => widget.field['FieldType']?.toString() ?? 'Text';
  String get _label =>
      widget.field['FieldLabel'] ?? widget.field['FieldName'] ?? '';
  bool get _isMandatory => widget.field['IsMandatory'] ?? false;
  ...
  void _initializeValues() {
    dynamic effectiveValue =
        widget.field['EnteredValue'] ?? widget.field['FieldValue'];
    if (effectiveValue == null ||
        (effectiveValue is String && effectiveValue.trim().isEmpty)) {
      effectiveValue = widget.field['DefaultValue'];
    }
    ...
    switch (_fieldType) {
      ...
      case 'CustomList':
      case 'SystemList':
        _selectedDropdownValue = widget.field['SelectedValue'];
        if (_selectedDropdownValue == null && effectiveValue != null) {
          final options = (widget.field['Options'] as List<dynamic>? ?? [])
              .whereType<CustomDropdownValue>()
              .toList();
          try {
            _selectedDropdownValue = options.firstWhere(
              (o) =>
                  o.valueId == effectiveValue.toString() ||
                  o.valueName == effectiveValue.toString(),
            );
          } catch (_) {}
        }
        _controller = TextEditingController(
          text: _selectedDropdownValue?.valueName ?? '',
        );
        break;
      ...
      case 'Date&Time':
        _controller = TextEditingController();
        _selectedDateTime = _parseDateTime(effectiveValue);
        _controller.text = _formatDateDisplay(_selectedDateTime);
        break;

      case 'MobileNumber':
        // ✅ Fix Bug 3: strip country code if already stored with it
        String rawPhone = effectiveValue?.toString() ?? '';
        for (final c in _countryCodes) {
          if (rawPhone.startsWith(c['code']!)) {
            _selectedCountryCode = c['code']!;
            _selectedCountryFlag = c['flag']!;
            rawPhone = rawPhone.substring(c['code']!.length).trim();
            break;
          }
        }
        _controller = TextEditingController(text: rawPhone);
        break;
  ...
  String _formatDateDisplay(DateTime? dt) {
    if (dt == null) return '';
    // ✅ Always convert to local before displaying
    final local = dt.toLocal();
    final fmt = controller.selectedFormat?.key ?? 'dd/MM/yyyy';
    if (_fieldType == "Date&Time") {
      return DateFormat('$fmt hh:mm a').format(local);
    }
    return DateFormat(fmt).format(local);
  }
  ...
  String? _validate(String? value) {
    final v = value?.trim() ?? '';
    if (_isMandatory && v.isEmpty) return '$_label is required';
    ...
    switch (_fieldType) {
      case 'Email':
        ...
        break;
      case 'MobileNumber':
        // ✅ Fix Bug 3: only digits, 6–12 length
        if (!RegExp(r'^\d{6,12}$').hasMatch(v)) {
          return 'Enter a valid mobile number (6–12 digits, no spaces)';
        }
        break;
      case 'URL':
        if (!RegExp(
  ...
  void _save() {
    ...
    if (_fieldType == 'Checkbox') {
      if (_isMandatory && !_checkboxValue) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('$_label is required')));
        return;
      }
      final value = _checkboxValue.toString();
      widget.field['EnteredValue'] = value;
      widget.field['FieldValue'] = value; // ✅ Also update FieldValue
      controller.updateCustomFieldValue(
        widget.lineIndex,
    ...
    // ── Date / DateTime ──────────────────────────────────────────────────────
    if (_fieldType == 'Date' || _fieldType == 'Date&Time') {
      if (_isMandatory && _selectedDateTime == null) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('$_label is required')));
        return;
      ...
      if (_selectedDateTime != null) {
        final value = toMillisecondsWithTimezone(_selectedDateTime!).toString();
        widget.field['EnteredValue'] = value;
        widget.field['FieldValue'] = value; // ✅ Also update FieldValue
        controller.updateCustomFieldValue(
          widget.lineIndex,
    ...
    // ── Dropdown ─────────────────────────────────────────────────────────────
    if (_fieldType == 'List' ||
        _fieldType == 'CustomList' ||
        _fieldType == 'SystemList') {
      if (_isMandatory && _selectedDropdownValue == null) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('$_label is required')));
        return;
      }
      final value = _selectedDropdownValue?.valueId ?? '';
      widget.field['SelectedValue'] = _selectedDropdownValue;
      widget.field['EnteredValue'] = value;
      widget.field['FieldValue'] = value; // ✅ Also update FieldValue
      controller.updateCustomFieldValue(
        widget.lineIndex,
    ...
    String finalValue = _controller.text.trim();

    if (_fieldType == 'MobileNumber') {
      finalValue = '$_selectedCountryCode $finalValue';
    }

    widget.field['EnteredValue'] = finalValue;
    widget.field['FieldValue'] = finalValue; // ✅ Also update FieldValue
  ...
  Widget _buildInput() {
    switch (_fieldType) {
        ...
        return SearchableMultiColumnDropdownField<CustomDropdownValue>(
          ...
          onChanged: (val) {
            setState(() {
              ...
              _controller.text = val?.valueName ?? '';
            });
          },
        );
      ...
      case 'Checkbox':
        return StatefulBuilder(
          builder: (_, inner) => CheckboxListTile(
            title: Text('$_label${_isMandatory ? " *" : ""}'),
            value: _checkboxValue,
            controlAffinity: ListTileControlAffinity.leading,
            contentPadding: EdgeInsets.zero,
            onChanged: (val) {
              inner(() => _checkboxValue = val ?? false);
              setState(() => _checkboxValue = val ?? false);
            },
          ),
        );

      // ✅ Fix Bug 3: Date — readOnly, only date picker allowed
      case 'Date':
      case 'Date&Time':
        final isDateTime = _fieldType == 'Date&Time';
        return TextFormField(
          ...
          decoration: InputDecoration(
            ...
            border: const OutlineInputBorder(),
            suffixIcon: const Icon(Icons.calendar_today),
            errorText: _isMandatory && _selectedDateTime == null
                ? '$_label is required'
                : null,
          ),
          onTap: () async {
            final picked = await showDatePicker(
              context: context,
              initialDate: _selectedDateTime?.toLocal() ?? DateTime.now(),
              firstDate: DateTime(2000),
              lastDate: DateTime(2100),
            ...
            setState(() {
              ...
              _controller.text = _formatDateDisplay(result);
            });
          },
        );
      // ✅ Fix Bug 3: Mobile — digitsOnly, country flag prefix
      case 'MobileNumber':
        return TextFormField(
          controller: _controller,
          keyboardType: TextInputType.phone,
      ...
      default:
        return TextFormField(
          ...
          maxLines: _fieldType == 'TextArea' ? 3 : 1,
          validator: _validate,
        );
    }
  }
...
class _TimeDetailsSheetState extends State<TimeDetailsSheet> {
  ...
  void initState() {
    super.initState();
    final key = toMillisecondsWithTimezone(
      DateTime(
...
class _TimeTrackerViewState extends State<TimeTrackerView> {
  ...
  Widget _actionBtn({
    ...
    String? actionKey, // NEW: identifies this button's action
  }) {
    return Obx(() {
      final isLoading = controller.isTimerActionLoading.value;
      final isThisLoading = controller.loadingAction.value == actionKey;
      final isDisabled = isLoading && !isThisLoading; // other button is loading
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: isDisabled
                ? Colors
                      .grey
                      .shade300 // ✅ proper grey when disabled
                : isThisLoading
