...
Future<void> _submitForm(bool value) async {
  ...
  for (int i = 0; i < itemizeControllers.length; i++) {
    ...
    for (var field in item.customFieldsItems) {
      ...
      if (fieldId != null && enteredValue != null && enteredValue.toString().isNotEmpty) {
    ...
    Map<String, dynamic> expenseTrans = {
      ...
      'TaxGroup': (item.selectedTax?.taxGroupId?.isNotEmpty ?? false) ? item.selectedTax!.taxGroupId : null,
      'LineAmountTrans': double.tryParse(item.lineAmount.text) ?? 0,
      'LineAmountReporting': double.tryParse(item.lineAmountINR.text) ?? 0,
      'ProjectId': item.projectDropDowncontroller.text.trim().isEmpty ? null : item.projectDropDowncontroller.text.trim(),
      'Description': item.descriptionController.text.trim(),
      'IsReimbursable': item.isReimbursable,
      'IsBillable': item.isBillableCreate,
      'AccountingDistributions': item.accountingDistributions.whereType<AccountingDistribution>().toList(),
      'ExpenseTransCustomFieldValues': itemCustomFieldValues,
      'ExpenseTransExpensecategorycustomfieldvalues': expenseCategoryCustomFields,
    };
...
Widget _buildCustomField({
  ...
  final String fieldKey = '${fieldKeyPrefix}_${field['FieldName']}_${field['LineId'] ?? 0}_${field['RecId'] ?? 0}';
  ...
  if (fieldType == 'Text' || fieldType == null || fieldType == '') {
    if (field['_textController'] == null) {
      ...
      (field['_textController'] as TextEditingController).addListener(() {
        final value = (field['_textController'] as TextEditingController).text;
  ...
  // For dropdown fields
  else if (fieldType == 'List' || fieldType == 'CustomList' || fieldType == 'SystemList') {
    ...
    if (selectedValue == null && field['DefaultValue'] != null) {
      final matches = options.where(
        (opt) => opt.valueId == field['DefaultValue'] || opt.valueName == field['DefaultValue'],
      );
    ...
    fieldController.text = selectedValue?.valueName ?? field['DefaultValue']?.toString() ?? '';
    ...
    if (selectedValue == null && field['DefaultValue'] != null) {
      ...
      final alreadyExists = options.any((opt) => opt.valueId == selectedValue!.valueId);
      if (!alreadyExists) {
    ...
    return Padding(
      ...
      child: Obx(() {
        ...
        return TextFormField(
          ...
          onTap: this.controller.isEnable.value ? () async {
            DateTime? currentDate = (field['_rxDateValue'] as Rx<DateTime?>).value ?? DateTime.now();
            ...
            if (isDateTime) {
              ...
              if ((field['_rxDateValue'] as Rx<DateTime?>).value != null) {
                initialTime = TimeOfDay.fromDateTime((field['_rxDateValue'] as Rx<DateTime?>).value!);
              }
              ...
              dateController.text = DateFormat('dd/MM/yyyy hh:mm a').format(fullDateTime);
            } else {
          ...
          validator: (value) {
            if (isMandatory && (field['_rxDateValue'] as Rx<DateTime?>).value == null) {
              return '$label is required';
            }
            return null;
          },
  ...
  // For numeric fields (LongInteger, Decimal)
    ...
    if (field['_numericController'] == null) {
      final initialValue = (field['_rxNumericValue'] as Rx<num?>).value?.toString() ?? '';
      ...
      (field['_numericController'] as TextEditingController).addListener(() {
        final value = (field['_numericController'] as TextEditingController).text;
        if (value.isEmpty) {
          ...
          if (numValue != null && numValue != (field['_rxNumericValue'] as Rx<num?>).value) {
  ...
  // For Email fields
    ...
    if (field['_emailController'] == null) {
      ...
      (field['_emailController'] as TextEditingController).addListener(() {
        final value = (field['_emailController'] as TextEditingController).text;
        if (value != (field['_rxStringValue'] as Rx<String?>).value) {
          (field['_rxStringValue'] as Rx<String?>).value = value;
          field['EnteredValue'] = value;
        }
  ...
  // For Checkbox fields
    ...
    if (field['_rxCheckboxValue'] == null) {
      final defaultValue = field['DefaultValue']?.toString().toLowerCase() == 'true';
      ...
      child: Obx(() {
        ...
        return Row(
          children: [
            Checkbox(
              value: rxValue.value,
              onChanged: this.controller.isEnable.value ? (val) {
